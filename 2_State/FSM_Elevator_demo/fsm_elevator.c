...
{
    printf("state@:idle!\n");
    printf("hint@:input target floor number[1, 9] : \n");
...
int event_goingUp(void)
{
    ...
    printf("hint@: goingup, current floor: %d\n", current_floor_number);
...
int event_goingDown(void)
{
    ...
    current_floor_number -= 1;
    accumulated_floor_number += 1;
    printf("hint@: goingdown, current floor: %d\n", current_floor_number);
...
{
    printf("state@:atTop!\n");
    printf("hint@: TOP, current floor: %d\n", current_floor_number);
    printf("hint@: reinput your target floor [1, 9] : \n");
    target_floor_number = getch() - '0';
...
int event_atBottom(void)
{
    ...
    printf("hint@: BOTTOM, current floor %d\n", current_floor_number);
    printf("hint@: reinput your target floor [1, 9] : \n");
    target_floor_number = getch() - '0';
...
int event_malfunction(void)
{
    ...
    printf("hint@: repair needed\n");
    printf("hint@: total arrived floor numbers: %d\n", accumulated_floor_number);
    return quit;
}
int event_end(void)
{
...
// The only thing possible flaw here is that the lookup_transitions will likely be linear (O(n)) 
// with this transition table datastructure. Its possible to make it better with multidimentional array - guaranteed O(1).
// Eg. the table can be represented as a multidimentional array where key is state and the value is an array 
// where the key is the return code and value is the next state: 
